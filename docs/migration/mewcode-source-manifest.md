...
- Scope:
  ...
  - `e2e_test.go` locks the MVP slim loop (requirements through closeout) with a static runner, a fake quality gate, and a fake review adapter, so the product path stays deterministic in CI.
