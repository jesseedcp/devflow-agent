...
- [ ] **Step 3: Add provider-neutral platform types**
...
- [ ] **Step 4: Add platform config structs and merge support**
...
- [ ] **Step 6: Commit platform contracts and config**
...
- [ ] **Step 5: Commit rendering helpers**
...
- [ ] **Step 3: Add platform doctor result helpers**
...
- [ ] **Step 1: Write failing GitHub issue intake tests**
...
- [ ] **Step 3: Implement GitHub issue intake**
...
- [ ] **Step 5: Commit GitHub issue intake adapter**
...
- [ ] **Step 1: Write failing CLI intake test**
...
- [ ] **Step 3: Extend intake flags and source loading**
...
- [ ] **Step 6: Commit GitHub Issue CLI intake**
...
- [ ] **Step 5: Write failing CLI sync test**
...
- [ ] **Step 6: Implement `devflow sync --github-issue`**
...
- [ ] **Step 8: Commit GitHub Issue sync**
...
- [ ] **Step 1: Write failing Feishu auth tests**
...
- [ ] **Step 3: Implement tenant token client**
...
- [ ] **Step 1: Write failing Feishu doc test**
...
func TestDocAdapterFetchesDocBlocksAsMarkdown(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case "/open-apis/docx/v1/documents/doc_token/blocks":
...
- [ ] **Step 3: Implement Feishu doc adapter**
...
func (a DocAdapter) FetchIntake(ctx context.Context, ref platform.IntakeRef) (platform.IntakeSnapshot, error) {
	...
	if err := a.getJSON(ctx, baseURL+"/open-apis/docx/v1/documents/"+url.PathEscape(token)+"/blocks", tenantToken, &blocks); err != nil {
...
- [ ] **Step 5: Commit Feishu doc adapter**
...
- [ ] **Step 6: Commit Feishu Doc CLI intake**
...
- [ ] **Step 1: Write failing Bitable tests**
...
func TestBitableAdapterListsDemands(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case "/open-apis/auth/v3/tenant_access_token/internal":
			json.NewEncoder(w).Encode(map[string]any{"code": 0, "tenant_access_token": "tenant-token", "expire": 7200})
		case "/open-apis/bitable/v1/apps/app_token/tables/tbl/records":
			json.NewEncoder(w).Encode(map[string]any{"code": 0, "data": map[string]any{"items": []map[string]any{{
				"record_id": "rec1",
...
- [ ] **Step 3: Implement Bitable adapter**
...
func (a BitableAdapter) listRecords(ctx context.Context, ref platform.IntakeRef) ([]bitableRecord, error) {
	...
	endpoint := a.normalizedBaseURL() + "/open-apis/bitable/v1/apps/" + url.PathEscape(ref.AppToken) + "/tables/" + url.PathEscape(ref.TableID) + "/records"
	if err := a.getJSON(ctx, endpoint, token, &parsed); err != nil {
		return nil, err
	}
	if parsed.Code != 0 {
...
- [ ] **Step 4: Run Feishu adapter tests**
...
- [ ] **Step 1: Write failing pool CLI test**
...
- [ ] **Step 4: Add Feishu Bitable intake**
...
- [ ] **Step 5: Add Feishu Bitable sync**
...
- [ ] **Step 6: Add CLI tests for Bitable intake and sync**
...
- [ ] **Step 8: Commit Feishu Bitable CLI**
...
- [ ] **Step 1: Add release-readiness fake smokes**
...
- [ ] **Step 2: Add platform config example**
...
- [ ] **Step 5: Run docs and script checks**
...
- [ ] **Step 6: Commit docs and release readiness**
...
- [ ] **Step 2: Run local CLI smoke commands**
...
- [ ] **Step 3: Update v0.2 release status**
