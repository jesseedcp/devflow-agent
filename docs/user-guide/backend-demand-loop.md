...
### Runtime Completion Finalizer

If `devflow run --stage implementation` reaches the runtime iteration limit after editing files and observing a passing test-like command, Devflow may write a deterministic `progress.md` summary. The summary explicitly says it was generated by the runtime finalizer and still passes through quality gates, implementation review, verification, and closeout.
