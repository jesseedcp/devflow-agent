...
func RenderProject(report ProjectMetrics) string {
	...
	b.WriteString("## Runtime Efficiency\n\n")
	fmt.Fprintf(&b, "- Runtime stages: %d\n", report.RuntimeStageCount)
	fmt.Fprintf(&b, "- Runtime tool calls: %d\n", report.RuntimeToolCallCount)
	fmt.Fprintf(&b, "- Deterministic finalizers: %d\n", report.RuntimeFinalizedCount)
	fmt.Fprintf(&b, "- Max-iteration finalizers: %d\n\n", report.RuntimeMaxIterationFinalizers)
